...
#include "Adafruit_TFT8_ILI9341.h"
...
/**************************************************************************/
/*!
    @brief  Instantiate Adafruit ILI9341 driver
    @param    cs    Chip select pin #
    @param    dc    Data/Command pin #
    @param    rst   Reset pin # (optional, pass -1 if unused)
*/
/**************************************************************************/
Adafruit_TFT8_ILI9341::Adafruit_TFT8_ILI9341(int8_t D0, int8_t WR, int8_t DC,
  int8_t CS, int8_t RST, int8_t RD, bool wide) : Adafruit_TFT8(
  ILI9341_TFTWIDTH, ILI9341_TFTHEIGHT, D0, WR, DC, CS, RST, RD, wide) {
}
...
*/
/**************************************************************************/
bool Adafruit_TFT8_ILI9341::begin(void) {
    if(!init()) return false; // MCU hardware init in TFT8 class

    startWrite(); // Chip select
    ...
    while((cmd = pgm_read_byte(addr++)) > 0) {
        ...
        numArgs = x & 0x7F;
        while(numArgs--) write8(pgm_read_byte(addr++));
        if(x & 0x80) delay(120);
    }
...
*/
/**************************************************************************/
void Adafruit_TFT8_ILI9341::setRotation(uint8_t m) {
...
*/
/**************************************************************************/
void Adafruit_TFT8_ILI9341::invertDisplay(boolean invert) {
    startWrite();
...
*/
/**************************************************************************/
void Adafruit_TFT8_ILI9341::scrollTo(uint16_t y) {
    startWrite();
...
*/
/**************************************************************************/
void Adafruit_TFT8_ILI9341::setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
...
*/
/**************************************************************************/
uint8_t Adafruit_TFT8_ILI9341::readcommand8(uint8_t command, uint8_t index) {
#if 0
    uint32_t freq = _freq;
